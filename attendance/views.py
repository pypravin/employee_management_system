...
from django.views.decorators.csrf import csrf_protect  # Better than exempting
...
def attendance_page(request):
    ...
    show_manual_attendance = request.GET.get(
        "facial_failed", "false") == "true"
...
def start_face_recognition(request):
    ...
    try:
        ...
        process = subprocess.Popen(
            [python_exec, script_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = process.communicate()

        output_message = stdout.decode(
            errors="ignore") if stdout else "Face recognition completed."
        ...
        if "No match found" in output_message or "Error" in output_message:
            ...
            return JsonResponse({"success": False, "message": "Face not recognized."}, status=200)
        ...
        if recognized_employee_id:
            try:
                ...
                attendance, created = Attendance.objects.get_or_create(
                    employee=employee, date=today, defaults={
                        "method": "Facial"}
                )
...
def search_employee(request):
    ...
    if query:
        employees = Employee.objects.filter(
            Q(first_name__istartswith=query) | Q(last_name__istartswith=query) | Q(
...
def record_attendance(request):
    ...
    try:
        ...
        if is_employee_on_leave(employee):
            return JsonResponse({
                "error": f"{employee.full_name} is on approved leave. Manual attendance cannot be recorded."
            }, status=400)
        ...
        # Create or fetch existing attendance
        attendance, created = Attendance.objects.get_or_create(
            employee=employee,
            date=date,
        ...
        try:
            time_in = datetime.datetime.strptime(
                time_in_str, "%H:%M").time() if time_in_str else None
            time_out = datetime.datetime.strptime(
                time_out_str, "%H:%M").time() if time_out_str else None
        except ValueError:
        ...
        if time_out:
            ...
            if attendance.time_out:
                return JsonResponse({"error": "Time Out already recorded for this employee today."}, status=400)
            if time_out <= attendance.time_in:
        ...
        elif time_out and not time_in_str:
            message = "Time Out recorded successfully."

        return JsonResponse({"success": True, "message": message})

    except Exception as e:
        print(f"[record_attendance error] {e}")
...
def attendance_list(request):
    ...
    attendance_records = Attendance.objects.select_related(
        "employee", "employee__department").all().order_by('-date', '-time_in')
    ...
    if department_id.isdigit():
        attendance_records = attendance_records.filter(
            employee__department_id=int(department_id))
...
def get_latest_attendance(request):
    ...
    try:
        ...
        data = []
        for record in attendance_records:
            time_in = record.time_in.strftime(
                "%H:%M:%S") if record.time_in else "-"
            time_out = record.time_out.strftime(
                "%H:%M:%S") if record.time_out else "-"
...
def employee_attendance_history(request):
    ...
    try:
        ...
        attendance_records = Attendance.objects.filter(
            employee=employee).order_by('-date', '-time_in')
        context = {
            'attendance_records': attendance_records,
        }
...
def absent_list(request):
    ...
    all_active_employees = Employee.objects.filter(
        is_active=True, is_deleted=False)

    # Get employees who have recorded attendance today
    present_employees_today = Attendance.objects.filter(
        date=today).values_list('employee_id', flat=True).distinct()
