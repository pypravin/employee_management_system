...
def redirect_user_based_on_role(user):
    ...
    if user.is_superuser:
        ...
        # Redirect HR users to HR portal
        return redirect('hr_portal:hr_dashboard')
    else:
...
def employee_dashboard(request):
    if request.user.first_login:
        return redirect('employee_portal:force_password_change')
    ...
    leave_history = Leave.objects.filter(
        employee=request.user).order_by('-applied_at')[:5]
    ...
    leave_status_counts = Leave.objects.filter(
        employee=request.user).values('status').annotate(count=Count('id'))
    pending_leaves = 0
    approved_leaves = 0
    rejected_leaves = 0
    cancelled_leaves = 0
...
class ForcePasswordChangeView(PasswordChangeView):
    ...
    success_url = reverse_lazy('employee_portal:employee_dashboard')
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, "Password changed successfully! You can now access your account.")
        return response
